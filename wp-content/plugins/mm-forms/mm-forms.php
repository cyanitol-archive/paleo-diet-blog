<?php
/*
Plugin Name: MM Forms Pro
Plugin URI: http://plugins.motionmill.com
Description: MM Forms Pro comes with the power of a tank, but drives like a bike. 
Author: Motionmill
Version: 2.2.3
...
class mm_forms {
	...
	{	
		...
		$this->recursive_remove_directory('../wp-content/plugins/mm-forms/upload');
		//delete_option('mmf');
	}
...
/* Admin panel */
	...
	function add_pages() {
		...
		elseif (isset($_POST['mmf-delete'])) {
         ...
         add_options_page(__('MM Forms Pro', 'mmf'), __('MM Forms Pro', 'mmf'), 'manage_options', __FILE__, array(&$this, 'option_page'));
	...
	function admin_head() {
		...
		if (isset($plugin_page) && $plugin_page == plugin_basename(__FILE__)) {
			$admin_stylesheet_url = get_option('siteurl') . '/wp-content/plugins/mm-forms/css/admin-stylesheet.css';
			echo '<link rel="stylesheet" href="' . $admin_stylesheet_url . '" type="text/css" />';						
			$admin_cal_url = get_option('siteurl') . '/wp-content/plugins/mm-forms/includes/ui/';
			echo '<link rel="stylesheet" href="' . $admin_cal_url . 'ui.all.css" type="text/css" />';			
			
			$js_path = get_option('siteurl') . '/wp-content/plugins/mm-forms/js/';
...
//		if($this->checkdateavailability($id) && $this->checkUserAuthentication($id))
    ...
    function wp_head()
    {
            ...
            $stylesheet_url = get_option('siteurl') . '/wp-content/plugins/mm-forms/css/stylesheet.css';
            $calendar_css = get_option('siteurl') . '/wp-content/plugins/mm-forms/css/calendar.css';
    ...
    if (is_admin() && 'options-general.php' == $pagenow && false !== strpos($_GET['page'], 'mm-forms'))
            ...
            if (! is_admin()) {
                    $jquery_form_script = get_option('site_url') . '/wp-content/plugins/mm-forms/js/jquery.form.js';
                    wp_enqueue_script('jquery-form', $jquery_form_script, array('jquery'), '');

                    $javascript_url = get_option('siteurl') . '/wp-content/plugins/mm-forms/js/mm-forms.js';
                    $javascript_url_ajax = get_option('siteurl') . '/wp-content/plugins/mm-forms/js/ajaxfileupload.js';

                    $calendar_js = get_option('siteurl') . '/wp-content/plugins/mm-forms/js/calendar.js';
...
/* Processing form element placeholders */
	...
	function form_element_replace_callback($matches) {
		...
		switch ($type) {
			...
			case 'fileField':
				...
				<img  id="thumb_image-'.$name.'" src="'.get_option("siteurl") . "/wp-content/plugins/mm-forms/images/transparent.gif".'" />
				<iframe width="0" height="0" style="visibility: hidden">
						 </iframe> <br> <iframe id="bfile" name="bfile" src="'.get_option('siteurl') . '/wp-content/plugins/mm-forms/includes/ajaxfileupload.php?ufiletypes='.$this->get_uploadfiletypes($fid).'&maxfilesize='.$this->get_maxfilesize($fid).'&fieldname='.$name.'" scrolling="no" frameborder="0" width="400" height="55"></iframe>';
	...
	function save_form_data($id, $form_data)
	{
		...
		foreach(array_keys($form_data) as $key)
		{
			...
			if(eregi("uploaded-file-", $key) and $arr_value != 'none') {
				...
				copy('wp-content/plugins/mm-forms/upload/temp/'.$arr_value, 'wp-content/plugins/mm-forms/upload/form-upload/'.$arr_value);
				unlink('wp-content/plugins/mm-forms/upload/temp/'.$arr_value);
				
				// copying tumbnail
				copy('wp-content/plugins/mm-forms/upload/temp/thumb_'.$arr_value, 'wp-content/plugins/mm-forms/upload/form-upload/thumb_'.$arr_value);
				unlink('wp-content/plugins/mm-forms/upload/temp/thumb_'.$arr_value);
	...
	function mm_plugin_links($links, $file) {
	    ...
	    if ($file == $this_plugin){
	        $settings_link = '<a href="admin.php?page=mm-forms/mm-forms.php">'.__("Settings").'</a>';
...
//      echo $sql ;
        ...
        if (!$results) {
                ...
                foreach ( $results as $res ) {
						...
						<a href='" . get_bloginfo('url') . "/wp-admin/options-general.php?page=mm-forms/mm-forms.php&action=viewDetail&id=" . $res->fk_form_joiner_id . "'>
						<img src='" . get_bloginfo('url') . "/wp-content/plugins/mm-forms/images/view.png' />
						</a>
						</th></tr>";
                }
        }
