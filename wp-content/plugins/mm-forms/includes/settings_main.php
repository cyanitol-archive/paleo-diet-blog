...
<div class="wrap relative" style="margin-top:15px;">
	<ul id="form_tab_container">
		...
		<li id="form_data_template" class="<?php echo ($tab == 'dt') ? 'current' : '' ?>"><a href="#" onclick="show_tab('form_data_template','form_tab_container','dt_tab','eo_tab,fo_tab,mo_tab,ms_tab,adv_tab')" title="<?php _e('frontend') ?>"><?php _e('frontend') ?></a></li>
		<li id="message_display" class="<?php echo ($tab == 'ms') ? 'current' : '' ?>"><a href="#" onclick="show_tab('message_display','form_tab_container','ms_tab','eo_tab,fo_tab,mo_tab,dt_tab,adv_tab')" title="<?php _e('notifications') ?>"><?php _e('notifications') ?></a></li>
		<li id="advanced_settings" class="<?php echo ($tab == 'adv') ? 'current' : '' ?>"><a href="#" onclick="javascript: 					
			 	show_tab('advanced_settings','form_tab_container','adv_tab','eo_tab,fo_tab,mo_tab,dt_tab,ms_tab'); document.getElementById('mmf-save').style.display='none';" title="<?php _e('copy') ?>"><?php _e('copy') ?></a></li>
	</ul>
	...
	<div style="width:98.7%;padding-left:10px;float:left;margin-top:0px;" class="settingsblock">
		...
		<div id="adv_tab" class="<?php echo ($tab =='adv') ? 'current_tab' : 'inactive_tab'?>" >
			<?php require_once $includes.'copy_settings.php'; ?>
		</div>	
	</div>
</div>
